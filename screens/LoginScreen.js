...
export default function LoginScreen({ navigation }) {
  ...
  const onLogin = async () => {
    try {
      if (email !== '' && password !== '') {
        const userTemp = await auth.signInWithEmailAndPassword(email, password);
        //console.log(userTemp.user.uid)
      }
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
     ...
     <Text style={{alignSelf: 'center', fontSize: 16, color: 'gray', fontWeight: '400', fontStyle: 'italic'}}>Order Management</Text>
